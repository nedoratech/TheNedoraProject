import { LEADING_BODY, TEXT_FOOTER } from '@/lib/sectionSpacing'
...
export default function Footer({ tagline, copyright, columns, locale }: FooterProps) {
  return (
    <footer className={`bg-nd-black text-nd-white border-t border-white/[0.06] ${TEXT_FOOTER}`}>
      <div className="max-w-[1160px] mx-auto px-5 sm:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-12 lg:gap-16 py-14 sm:py-16 border-b border-white/[0.06]">
          <div>
            ...
            <p className={`text-white/40 ${LEADING_BODY} max-w-[280px]`}>{tagline}</p>
          </div>

          {columns.map((col) => (
            <div key={col.label}>
              <div className="tracking-[0.2em] uppercase font-bold text-white/30 mb-5">{col.label}</div>
                ...
                {col.links.map((link) => (
                  <li key={link.text}>
                    {link.external ? (
                      ...
                      <a href={link.href} className="text-white/50 hover:text-nd-white transition-colors duration-200">
                        {link.text}
                      </a>
                    ) : (
                      <Link href={link.href} className="text-white/50 hover:text-nd-white transition-colors duration-200">
                        {link.text}
                      </Link>
                    )}
                  </li>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-6">
          <span className="text-white/25 tracking-[0.05em]">{copyright}</span>
          <div className="flex gap-1 tracking-[0.1em] uppercase font-bold">
