...
import { LEADING_BODY, LEADING_HEADING } from '@/lib/sectionSpacing'
...
export default function PrivacyPolicy({
  ...
  return (
    <article className="max-w-[720px] mx-auto px-5 sm:px-8 py-14 sm:py-18 lg:py-24">
      ...
      <h1 className={`text-[2.25rem] sm:text-[2.5rem] font-bold tracking-[-0.035em] ${LEADING_HEADING} text-nd-black mb-4`}>
        {title}
      </h1>
      <p className="text-[0.75rem] tracking-[0.08em] text-nd-grey-400 mb-10">{updated}</p>

      <p className={`text-[0.95rem] text-nd-grey-600 ${LEADING_BODY} mb-12`}>{intro}</p>
          ...
          <section key={section.id} id={section.id} className="scroll-mt-[calc(var(--nd-nav-h)+1.5rem)]">
            ...
            {section.paragraphs.map((paragraph, index) => (
              <p
                ...
                className={`text-[0.9rem] text-nd-grey-600 ${LEADING_BODY} mb-4 last:mb-0`}
              >
                {paragraph}
              </p>
            ))}
            {section.list && section.list.length > 0 && (
              <ul className="flex flex-col gap-2 mt-2 border-l-2 border-nd-accent-mid/40 pl-5">
                {section.list.map((item, index) => (
                  <li key={`${section.id}-l-${index}`} className={`text-[0.9rem] text-nd-grey-600 ${LEADING_BODY}`}>
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <div className="mt-14 border border-nd-grey-200 bg-nd-grey-50 px-6 py-8">
        ...
        <p className={`text-[0.9rem] text-nd-grey-600 ${LEADING_BODY} mb-4`}>{contactBody}</p>
        <a
